...
function confirmDelete(itemName, itemUrl) {
    console.log('confirmDelete called with:', itemName, itemUrl);
    
    // Escape any quotes in the item name to prevent JavaScript errors
    const escapedItemName = itemName.replace(/'/g, "\\'").replace(/"/g, '\\"');
    
    if (confirm(`Are you sure you want to delete "${escapedItemName}"? This action cannot be undone.`)) {
        console.log('User confirmed deletion, redirecting to:', itemUrl);
